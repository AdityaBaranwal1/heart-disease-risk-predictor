...
Comprehensive launcher for the Heart Disease Prediction System
...
class CS210ProjectLauncher:
    ...
    def start_enhanced_web_interface(self):
        """Start the enhanced web interface if it exists."""
        ...
        if web_file.exists():
            try:
                ...
                self.processes.append(('Enhanced Web Interface', process))
                print("✅ Enhanced Web Interface started on http://localhost:5000")
                time.sleep(2)
                return True
                
            except Exception as e:
                print(f"❌ Failed to start enhanced web interface: {e}")
                return False
        else:
            print("ℹ️ Enhanced web interface not found, skipping...")
            return True
    ...
    def open_browsers(self):
        ...
        # Open API documentation (if enhanced interface is not running)
        if not any('Enhanced Web Interface' in name for name, _ in self.processes):
            try:
    ...
    def display_access_info(self):
        ...
        if any('Enhanced Web Interface' in name for name, _ in self.processes):
    ...
    def run(self):
        ...
        # Start enhanced web interface (if available)
        self.start_enhanced_web_interface()
