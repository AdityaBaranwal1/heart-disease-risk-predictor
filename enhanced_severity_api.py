"""
Improved Heart Disease Severity Predictor API
...
class ImprovedHeartDiseasePredictorAPI:
    def __init__(self):
        """Initialize the improved predictor with both binary and severity models."""
        ...
        try:
            ...
            self.logger.info("Improved Heart Disease Predictor API initialized successfully")
    ...
    def _load_or_train_models(self):
        ...
        binary_model_path = 'models/improved_binary_model.joblib'
        severity_model_path = 'models/improved_severity_model.joblib'
        scaler_path = 'models/improved_scaler.joblib'
        ...
        if (os.path.exists(binary_model_path) and
            ...
            self.logger.info("Loaded existing improved models")
        else:
            # Train new models
            self._train_improved_models()

    def _train_improved_models(self):
        ...
        joblib.dump(self.binary_model, 'models/improved_binary_model.joblib')
        joblib.dump(self.severity_model, 'models/improved_severity_model.joblib')
        joblib.dump(self.scaler, 'models/improved_scaler.joblib')

        self.logger.info("Improved models trained and saved successfully")
    ...
    def _setup_routes(self):
        ...
        def predict():
            """Improved prediction endpoint with severity levels."""
            try:
                # Get input data
    ...
    def run(self, host='127.0.0.1', port=5000, debug=True):
        ...
        self.logger.info(f"Starting Improved Heart Disease Predictor API on {host}:{port}")
        self.app.run(host=host, port=port, debug=debug)

if __name__ == '__main__':
    api = ImprovedHeartDiseasePredictorAPI()
    api.run()
